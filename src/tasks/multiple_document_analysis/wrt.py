...
from src.tasks.multiple_document_analysis.wrt_llm import text_splitter, summary_wrt_chain, final_violation_chain

from langchain.docstore.document import Document as LangchainDoc


def run_multiple_document_wrt_analysis(db_analysis : MultipleDocumentAnalysis):
    ...
    while(failure_count<5):
        ...
        try:
            ...
            result_text = summary_wrt_chain.invoke({"primary": analysis_one_summary, "secondary": analysis_two_summary})

            summary_violations = final_violation_chain({"summary": result_text})

            value = {
                "summary": result_text,
                "inconsistencies": summary_violations['text']['violations'],
            }
