...
from src.tasks.single_document_analysis.hipaa_llm import text_splitter, hipaa_summarizer_chain, final_inconsistency_chain

from langchain.docstore.document import Document as LangchainDoc


def reset_and_run_broken_hipaa_analyses():
    db : Session = next(get_db())

    broken_jobs = db.exec(
        select(SingleDocumentAnalysis)
        .where(SingleDocumentAnalysis.kind == SingleDocumentAnalysisKinds.HIPAA)
    ...
    for job in broken_jobs:
        run_hipaa_analysis(job)

def request_hipaa_analysis(document_id):
    db : Session = next(get_db())
    ...
    db_analysis = SingleDocumentAnalysis(
        ...
        kind=SingleDocumentAnalysisKinds.HIPAA,
    ...
    run_hipaa_analysis(db_analysis)

def run_hipaa_analysis(db_analysis : SingleDocumentAnalysis):
    db : Session = next(get_db())
    ...
    while(failure_count<5):
        ...
        try:
            ...
            result_text = hipaa_summarizer_chain.run(split_docs)
