...
from src.tasks.single_document_analysis.coppa_llm import text_splitter, coppa_summarizer_chain, final_inconsistency_chain

from langchain.docstore.document import Document as LangchainDoc


def reset_and_run_broken_coppa_analyses():
    db : Session = next(get_db())

    broken_jobs = db.exec(
        select(SingleDocumentAnalysis)
        .where(SingleDocumentAnalysis.kind == SingleDocumentAnalysisKinds.COPPA)
    ...
    for job in broken_jobs:
        run_coppa_analysis(job)

def request_coppa_analysis(document_id):
    db : Session = next(get_db())
    ...
    db_analysis = SingleDocumentAnalysis(
        ...
        kind=SingleDocumentAnalysisKinds.COPPA,
    ...
    run_coppa_analysis(db_analysis)

def run_coppa_analysis(db_analysis : SingleDocumentAnalysis):
    db : Session = next(get_db())
    ...
    while(failure_count<5):
        ...
        try:
            ...
            result_text = coppa_summarizer_chain.run(split_docs)
